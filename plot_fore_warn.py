...
from model import PngWarnWeatherTable
...
variable_info = {
  'T2max': {
      ...
      'unit': '°C',
      'levels':[35, 37, 40],
      'colors':('#FFFFFF','#FFBFBE','#FF5500','#E60000'),
      'type': 1,
      'period': 3
  },
    'T2min': {
      ...
      'title': '全国低温预警图',
      'unit': '°C',
      'levels':[-7, -5, -3, -1, 0, 1, 2],
      'colors':('#003087','#1C5CA6','#2175CF','#3E9FEC','#81D2FF','#ACE7F7','#D2FBFF','#FFFFFF'),
      'type': 2,
      'period': 3
  }
}
...
def plot_tmpmax_legend(var_details,ax):
  '''
  添加气温图例
  ...
  text_y = 0.133 # '图例'文字垂直边距
  # 图例文字内容
  color_labels = [("35~37","#FFBFBE"),("37~40","#FF5500"),("≥40","#E60000")]
  color_x0 = 0.155 # 左上角第1个矩形色标x坐标
  color_y0 = 0.095 # 左上角第1个矩形色标y坐标
  ...
  # ---------- 图例绘制 ----------
  # 添加左下角的图例框
  legend_box = patches.Rectangle((0.15, 0.01), 0.12, 0.16, linewidth=0.6, edgecolor='lightgrey', 
                                facecolor='white', transform=ax.transAxes, zorder=10)
  ax.add_patch(legend_box)
  # 添加图例文字-图例
  plt.text(text_x, text_y, "图例", fontname='SimHei', fontsize=9, transform=ax.transAxes, zorder=15)
  # 添加图例文字-单位,相较'图例'向右偏移0.04
  plt.text(text_x + 0.06, text_y, f"({var_unit})", fontname='Times New Roman', fontsize=9, transform=ax.transAxes, zorder=15)
  # 设置不同范围的数字及其对应的颜色
  for i, (label, color) in enumerate(color_labels):
    rect = patches.Rectangle((color_x0, color_y0 - i * gap), color_l, color_h, linewidth=0.2, edgecolor='grey',
                            facecolor=color, transform=ax.transAxes, zorder=15)
    ax.add_patch(rect)
    ax.text(color_x0+color_l+0.005, color_y0+0.006 - i * gap, label, fontsize=6.5, transform=ax.transAxes, verticalalignment='center', fontfamily='Times New Roman',zorder=15)

  print("(6/8) plot_tmpmax_legend --> finished!")


def plot_tmpmin_legend(var_details,ax):
  '''
  添加气温图例
  ...
  # 图例文字内容
  color_labels = [("<-7", "#003087"), ("-7~-5", "#1C5CA6"), ("-5~-3", "#2175CF"), ("-3~-1", "#3E9FEC"),("-1~0", "#81D2FF"), ("0~1", "#ACE7F7"),("1~2","#D2FBFF"),(">2","#FFFFFF")]
  color_x0 = 0.155 # 左上角第1个矩形色标x坐标
  ...
  # ---------- 图例绘制 ----------
  # 添加左下角的图例框
  legend_box = patches.Rectangle((0.15, 0.01), 0.09, 0.21, linewidth=0.6, edgecolor='lightgrey', 
                                facecolor='white', transform=ax.transAxes, zorder=10)
  ax.add_patch(legend_box)
  # 添加图例文字-图例
  ...
  print("(6/8) plot_tmpmin_legend --> finished!")
...
def plot_variable(yyyymmdd, variable, data, lat, lon, output_folder, output_sub_directory):
  ...
  try:
    ...
    # 6.添加特定变量的图例
    specific_plot_funcs = {
      'T2max': plot_tmpmax_legend,
      'T2min': plot_tmpmin_legend,
    }
    ...
    # 7.保存绘制的图像
    save_path = os.path.join(output_folder, f"{variable}_warn_{yyyymmdd}.png")
    fig.savefig(save_path)
    plt.close(fig)
    print(f"(7/8) Plot saved: {save_path}")
    
    # 8.生成入库信息
    save_sub_path = os.path.join(output_sub_directory, f"{variable}_warn_{yyyymmdd}.png")
...
def calculate_daily_stats(variable,ds,open_downsample=False,downsample_factor=4):
  ...
  - ds: xr.Dataset, 预警日期的一日数据, time*lat*lon
...
def get_forecast_date(ndays=1):
  ...
  - day_range: list, 未来ndays天(含当日)的日期字符序列, 例如['20230101', '20230102', '20230103']
  '''
...
def main():
  ...
  # 获取时间信息  
  fore_start_date, day_range = get_forecast_date()
  yyyymmdd    = day_range[0]
  # yyyymmdd = '20240910'
  
  # ---------- 输出检查 ----------
  output_sub_directory = os.path.join('pic_module/met_warn_GJD',yyyymmdd[0:4], yyyymmdd[0:6], yyyymmdd)
  ...
  session_merge_from_records(session, PngWarnWeatherTable, image_info_list, **add_kwargs)
  session.close()
...
if __name__ == "__main__":
  # 脚本与相应配置文件请放置在同一个路径下
  config_path = './'
  config_name = 'fore_warn_config.yaml'
