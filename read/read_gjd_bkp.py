...
class Reader_GJD(object):
    ...
    def __extract_file_infos__(self):
        ...
        if self.time_check:
            for folder, fb in zip(self.folder, self.folder_fbs):
                print("———————— ————————")
                print(f"查询目录: {folder} \t 发报时间: {fb}")
                
                yb_start = pd.to_datetime(fb, format=self.time_fmts['time_fb'])
                ...
                for i in range(len(self.extract_yb_range)):
                    ...
                    files = self.find_files(folder, yb_start_i, yb_stop_i, ignore_na=False)
    ...
    def find_files(self, folder:str, yb_start:pd.Timestamp, yb_stop:pd.Timestamp, ignore_na=False):
        files = []
        ...
        try:
            fbstr = re.match(self.folder_re, folder).groups()[0]
        except Exception as e:
            print(f"[ERROR] 无法匹配 {folder} 的时间: {e}")
            return None
        
        for ybstr in ybstr_range:
            filename_tar = self.file_name.format(time_fb=fbstr, time_yb=ybstr)
            path_tar = os.path.join(self.root, folder, filename_tar)
    ...
    def __get_folders_info__(self):
        ...
        folders_all = [f for f in os.listdir(self.root) if os.path.isdir(os.path.join(self.root, f))]
        folders_all = reversed(sorted(folders_all))
        
        for f in folders_all:
            m = re.match(self.folder_re, f)
